...
Веб-приложение полностью работает, все контейнеры запущены, база данных подключена. Это подтверждается выводом curl (Почему то не могу зайти в браузер на линуксе, поэтому проверяю через терминал)
